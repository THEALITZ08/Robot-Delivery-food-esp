...
const long intervalPackage = 1000;
const long intervalRecovery = 500;
...
TaskHandle_t ultrasonikTaskHandle = NULL;
...
void setup() {
  ...
  ArduinoOTA.setHostname("Robot-Delivery");

  ArduinoOTA.onStart([]() {
    otaMode = true;
    vTaskSuspend(fsmTaskHandle);
    vTaskSuspend(monitoringTaskHandle);
    vTaskSuspend(ultrasonikTaskHandle);
    stop(0,0);
    display.clearDisplay();
  ...
  ArduinoOTA.onProgress([](unsigned int progress, unsigned int total) {
    ...
    display.fillRect(0,25,persen,10,WHITE);
    display.display();
  ...
  ArduinoOTA.onEnd([]() {
    display.clearDisplay();
    display.setCursor(0,0);
    display.println("OTA SUCCESFULL");
